...
from playwright.async_api import async_playwright
import datetime
Path("/app/salidas").mkdir(parents=True, exist_ok=True)
...
async def extract_exante_title(url: str) -> str:
    ...
    try:
        async with async_playwright() as p:
            ...
            # Dump siempre que no se haya devuelto un título
            timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
            html_path = f"/app/salidas/debug_exante_{timestamp}.html"
...
async def extract_news_headlines_from_file(file: UploadFile) -> List[Dict[str, str]]:
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        ...
        for i in range(0, len(results), BATCH_SIZE):
            ...
            logger.info(f"Procesando lote {i//BATCH_SIZE + 1} de {(len(results) + BATCH_SIZE - 1)//BATCH_SIZE} del archivo {file.filename}")
